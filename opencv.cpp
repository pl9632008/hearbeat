...
//            result = diff == 0 ? std::numeric_limits<double>::max() : t.rows/diff;
            ...
            result = diff == 0 ? 30.0 : t.rows/diff;
    ...
    void bandpass(cv::InputArray _a, cv::OutputArray _b, double low, double high) {
        ...
        if (a.total() < 3) {
            ...
            Mat frequencySpectrum = Mat(a.rows, a.cols, CV_64F);
    ...
    void butterworth_lowpass_filter(Mat &filter, double cutoff, int n) {
        ...
        Mat tmp = Mat(filter.rows, filter.cols, CV_64F);
        ...
        for (int i = 0; i < filter.rows; i++) {
            for (int j = 0; j < filter.cols; j++) {
                ...
                tmp.at<double>(i, j) = (double)(1 / (1 + pow(radius / cutoff, 2 * n)));
            }
        }
    ...
    void timeToFrequency(InputArray _a, OutputArray _b, bool magnitude) {
        ...
        // Prepare planes
        Mat a = _a.getMat();
        Mat planes[] = {cv::Mat_<double>(a), cv::Mat::zeros(a.size(), CV_64F)};
        Mat powerSpectrum;
...
//            cv::normalize(magnitude, magnitude, 1, 0, NORM_L1, -1, bandMask);
            ...
            cv::normalize(magnitude, magnitude, 1, 0, NORM_L2, -1, bandMask);
